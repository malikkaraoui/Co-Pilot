...
class TestAllScenariosE2E:
    ...
    def test_scenario_has_eleven_filters(self, client, scenario):
        """Les 11 filtres L1-L11 sont presents dans chaque reponse."""
        resp = _analyze(client, scenario)
        body = resp.get_json()
        filter_ids = sorted(f["filter_id"] for f in body["data"]["filters"])
        assert filter_ids == ["L1", "L10", "L11", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "L9"]
...
class TestScenarioIncomplet:
    ...
    def test_l6_warning_no_phone_private(self, client):
        """Particulier sans tel → warning (red flag)."""
        resp = _analyze(client, SCENARIO_INCOMPLET)
        body = resp.get_json()
        l6 = _get_filter(body["data"]["filters"], "L6")
        assert l6["status"] == "warning"
