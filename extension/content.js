...
function buildL10Body(f, d) {
  ...
  // Position du curseur sur la barre (0% = 0 jours, 100% = 2x seuil)
...
function buildL9Body(f, d, allFilters) {
  ...
  // Couverture : compter les filtres évalués (hors L9 lui-même)
...
// ── L5 : Indice de confiance (échelle bipolaire) ────────
...
function buildL5Body(f, d) {
  ...
  // Position du curseur sur l'échelle 0-100 (50 = centre/RAS)
  // z_scores.price détermine la position :
  //   outlier (|z|>3) → extrême gauche (10) ou droite
  //   en marge (|z| 2-3) → gauche (25) ou droite (75)
  //   RAS (|z|<2, pas d'anomalies) → centre-droite (60-80)
  //   clean + beaucoup de refs → droite (80+)
  ...
  if (hasOutlier) {
    ...
    // RAS ou juste diesel urbain (pas une anomalie de prix)
    const bonus = Math.min(refCount, 20) / 20 * 20; // 0-20 points bonus selon refs
    cursorPct = 60 + bonus;
  ...
  // Échelle bipolaire SVG-like en HTML/CSS
  ...
  // Alerte diesel urbain (TOUJOURS affichée si applicable, même si RAS)
  if (d.diesel_urban) {
...
function buildPriceBarHTML(details, vehicle) {
  ...
  // Footer : source badge + sample count + precision stars
  ...
  // Banniere "anguille sous roche" : prix bas + annonce vieille
  let staleHTML = "";
