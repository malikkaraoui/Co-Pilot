/**
 * Bannieres CTA injectees en bas de la popup principale.
 * Chaque fonction retourne du HTML statique — pas de logique DOM ici,
 * c'est dom.js qui branche les listeners ensuite.
 */
...
/**
 * Section premium avec effet blur + overlay glass.
 * Le contenu flou simule un rapport detaille pour donner envie.
 * Le bouton est branche dans dom.js (affiche "Bientot disponible").
 * @returns {string} HTML de la section premium
 */
...
/**
 * Banniere video YouTube liee au modele du vehicule.
 * On tronque le titre a 50 chars pour garder la banniere compacte.
 * @param {Object|null} featuredVideo - {url, title, channel} du backend
 * @returns {string} HTML de la banniere ou chaine vide
 */
...
/**
 * Banniere Autoviza — rapport d'historique gratuit offert par LeBonCoin.
 * N'apparait que si le backend a detecte un lien Autoviza valide.
 * @param {string|null} autovizaUrl - URL du rapport Autoviza
 * @returns {string} HTML de la banniere ou chaine vide
 */
...
/**
 * Banniere de generation d'email au vendeur via Gemini.
 * Flow : bouton -> spinner -> textarea avec le brouillon OU erreur.
 * Les listeners (clic, copier) sont branches par dom.js apres injection.
 * @returns {string} HTML de la section email
 */
