...
export function buildL3Body(f, d) {
  ...
  const statHTML = `
    <div class="okazcar-l3-stat">
      ...
      <span class="okazcar-l3-expected">Attendu : ~${fmtKm(avgExpected || 15000)} km/an pour un véhicule de ${age} an${age > 1 ? "s" : ""}</span>
    </div>
  `;
  ...
  const barHTML = `
    <div class="okazcar-l3-comparison">
      <div class="okazcar-l3-bar-row">
        <span class="okazcar-l3-bar-label">Réel</span>
  ...
  let verdictHTML = "";
  if (f.status === "pass") {
    verdictHTML = '<div class="okazcar-l3-verdict okazcar-l3-ok">Kilométrage cohérent avec l\'âge du véhicule</div>';
  } else if (isRecentLowKm && isPro) {
    verdictHTML = '<div class="okazcar-l3-verdict okazcar-l3-warn">Véhicule quasi-neuf — probable immatriculation constructeur</div>';
  } else if (isRecentLowKm) {
    verdictHTML = '<div class="okazcar-l3-verdict okazcar-l3-warn">Véhicule quasi-neuf — n\'a pas trouvé preneur</div>';
  } else if (kmRatio < 0.5) {
    verdictHTML = '<div class="okazcar-l3-verdict okazcar-l3-alert">Kilométrage très bas — compteur remis à zéro ?</div>';
  } else if (kmRatio > 2.0) {
    verdictHTML = '<div class="okazcar-l3-verdict okazcar-l3-alert">Kilométrage très élevé — usure accélérée</div>';
  } else {
    verdictHTML = '<div class="okazcar-l3-verdict okazcar-l3-warn">Kilométrage à surveiller</div>';
  }

  let proHTML = "";
  if (isPro) {
    proHTML = '<span class="okazcar-l3-pro-badge">Véhicule pro</span>';
  }
